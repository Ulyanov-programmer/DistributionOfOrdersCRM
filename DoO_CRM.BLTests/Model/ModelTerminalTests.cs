...
namespace DoO_CRM.BLTests.Model
{
    ...
    public class ModelTerminalTests
    {
        ...
        public void SaveSellsTests()
        {
            //Arrange
            var context = new DoO_CRMContext();
            var client = ClientController.Registration($"Пользователь {Guid.NewGuid()}", 500000, context);
            ...
            */
            ...
            //foreach (var sell in savedSells_fromCart)
            //{
            //    foreach (var sellFromDB in savedSells_fromDb)
            //    {
            //        Assert.IsTrue(sell.Equals(sellFromDB));
            //    }
            //}
        }
    }
